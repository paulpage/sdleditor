...
impl<'a> App<'a> {
    ...
    fn active_buffer(&mut self) -> &mut Buffer {
        &mut self.buffers[self.panes[self.pane_idx].buffer_id]
    }

    fn draw(&mut self, mut canvas: &mut WindowCanvas) {
        ...
        for pane in &mut self.panes {
            ...
            for (i, entry) in buffer.contents[first_line..last_line].iter().enumerate().map(|(i, entry)| (i + first_line, entry)) {
                ...
                let line_y = bar_height + padding + i as i32 * pane.line_height as i32 - pane.scroll_offset;
    ...
    fn main() {
        ...
        if args.len() > 1 {
            for arg in &args[1..] {
                app.buffers.push(Buffer::from_path(arg.to_string()));
            }
        } else {
                    ...
                    Event::KeyDown {
                        ...
                        if keymod.contains(Mod::RCTRLMOD) || keymod.contains(Mod::LCTRLMOD) {
                            if keymod.contains(Mod::RSHIFTMOD) || keymod.contains(Mod::LSHIFTMOD) {
                                match kc {
                                    Keycode::Backslash => {
                                        app.active_buffer().print();
                                        break 'mainloop;
                                    }
                                    Keycode::Z => {
                                        app.active_buffer().redo();
                                    }
                                    _ => {}
                                }
                            } else {
                                match kc {
                                    ...
                                    Keycode::Z => {
                                        app.active_buffer().undo();
                                    }
                                    Keycode::S => {
                                        app.active_buffer().save();
                                    }
                                    Keycode::O => {
                                    ...
                                    Keycode::Tab => {
                                        ...
                                        let b = pane.buffer_id;
                                        let new_id = if b < app.buffers.len() - 1 { b + 1 } else { 0 };
                                        pane.buffer_id = new_id;
                                    }
                                    Keycode::K => {
                                        app.panes.push(Pane::new(200, 200, 400, 400, ttf_context.load_font("data/LiberationSans-Regular.ttf", 16).unwrap(), PaneType::FileManager, 0));
                                    }
                                    _ => {
                                        let b = app.panes[app.pane_idx].buffer_id;
                                        handle_buffer_event(&mut app.panes[app.pane_idx], &mut app.buffers[b], event);
                                    }
                                }
                            }
                        } else {
