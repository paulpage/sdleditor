    ...
    fn handle_buffer_event(&mut self, buffer: &mut Buffer, event: Event) {
        match event {
            ...
            Event::MouseButtonDown { x, y, .. } => {
                ...
                let padding = 5;
                let mut y_idx = ((f64::from(y) - f64::from(self.y) - f64::from(padding) - f64::from(bar_height)) / f64::from(self.line_height)).floor() as usize + self.scroll_idx;
                ...
                while char_x < x && (x_idx as usize) < max_x_idx + 1 {
                    let (cx, _) = self.font.size_of(&buffer.contents[self.cursor_y][..x_idx]).unwrap();
                    last_char_x = char_x;
